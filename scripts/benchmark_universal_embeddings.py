...
def _control_matrix(frame: pd.DataFrame) -> tuple[np.ndarray, list[str]]:
    ...
    for row_index, (row_names, row_values) in enumerate(zip(frame["parameter_names"], frame["parameter_values"])):
...
def _quality(
    ...
    return {
        ...
        "parameter_distance": float(np.mean(np.linalg.norm(controls[queries] - controls[neighbors], axis=1))),
        "shape_distance": float(np.mean(np.linalg.norm(bitmaps[queries] - bitmaps[neighbors], axis=1))),
        "mutual_capacitance_mae_ff": float(np.mean(np.abs(capacitance[queries] - capacitance[neighbors]))),
    }
...
def main() -> None:
    ...
    v0 = pd.read_parquet(args.v0_parquet).query("component_name == 'GeneralizedCapNInterdigital'")
    v1 = pd.read_parquet(args.v1_parquet).query("component_name == 'GeneralizedCapNInterdigital'")
    paired = v1.merge(
    ...
    paired = paired.iloc[rng.choice(len(paired), args.sample_size, replace=False)].reset_index(drop=True)
    ...
    capacitance_by_source = {row["notes"]["source_id"]: row["sim_results"]["north_to_south"] for row in rows}
    capacitance = np.asarray([capacitance_by_source[source_id] for source_id in paired["source_id"]])
    ...
    gates = {
        "topology": quality["v1"]["finger_count_exact"] >= quality["v0"]["finger_count_exact"],
        "parameters": quality["v1"]["parameter_distance"] <= 0.90 * quality["v0"]["parameter_distance"],
        "shape": quality["v1"]["shape_distance"] <= 1.10 * quality["v0"]["shape_distance"],
        "physics_proxy": quality["v1"]["mutual_capacitance_mae_ff"] <= quality["v0"]["mutual_capacitance_mae_ff"],
