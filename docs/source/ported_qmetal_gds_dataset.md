...
| `1/11` | all subtractive poly/path qgeometry | all subtractive poly/path qgeometry |
| `1/0` | omitted, as in the existing sweep | main-chip ground minus etch |
...
outward by 2 um. Its transverse width is the native pin/junction width. CapN's
...
GDS conductor and etch against Qiskit Metal with a symmetric-difference test,
checks marker geometry and unique conductor assignment, and confirms that the
universal v2 encoder discovers exactly two terminals:
...
write `metadata/layer-semantics-v1.json` schema 1.2.0 with the new roles.
